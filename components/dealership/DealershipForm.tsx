...
import { Send, CheckCircle2, AlertCircle, Package, Shield } from "lucide-react";
...
export default function DealershipForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // District validation
    if (!selectedDistrict) {
      setErrorMessage("Please select your primary district.");
      setSubmitStatus("ERROR");
      setIsSubmitting(false);
      return;
    }
    ...
    // Product check
    if (productsInterested.length === 0) {
      setErrorMessage("Please select at least one product of interest.");
      setSubmitStatus("ERROR");
      setIsSubmitting(false);
      return;
    }

    const combinedDistrictString = `${selectedDistrict}, ${selectedState}`;
    ...
    try {
      const response = await fetch("/api/partnerships/apply", {
        ...
        body: JSON.stringify({
          type: "DEALERSHIP",
          ...formData,
          primaryDistrict: combinedDistrictString,
          productsInterested,
        }),
      });
      ...
      console.error("Dealership submit error:", error);
      setSubmitStatus("ERROR");
      ...
      <div className="bg-white p-8 sm:p-12 rounded-3xl border border-slate-200 shadow-xl text-center space-y-6 max-w-2xl mx-auto">
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-600 leading-relaxed text-sm sm:text-base">
            Thank you for applying to become an authorized dealer. Our distribution team will review your application, verify your GSTIN details (if provided), and contact you with wholesale pricing, product catalogs, and credit terms within 24–48 hours.
          </p>
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-10 rounded-3xl border border-slate-200 shadow-xl max-w-3xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {/* Business Name */}
        <div>
          <label htmlFor="businessName" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
            Business / Shop Name *
          </label>
          <input
            type="text"
        ...
        {/* Contact Person Name */}
        <div>
          <label htmlFor="contactPersonName" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
            Contact Person Name *
          </label>
          <input
            type="text"
        ...
        {/* GSTIN */}
        <div>
          <label htmlFor="gstin" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
            GSTIN (Optional)
          </label>
          <input
            type="text"
            ...
            className="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-mono font-medium text-slate-900 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 focus:outline-none transition-all uppercase"
          />
        </div>
        ...
        {/* Primary District / Market Coverage Select */}
        <div>
          <label htmlFor="primaryDistrict" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
            Primary District / Market Coverage *
          </label>
          <select
            id="primaryDistrict"
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {productOptions.map((product) => {
            const isChecked = productsInterested.includes(product);
            return (
              <button
                type="button"
                key={product}
                onClick={() => handleCheckboxChange(product)}
                className={`flex flex-col items-center justify-center p-4 border rounded-2xl transition-all cursor-pointer select-none text-center ${
                  isChecked
                    ? "border-emerald-600 bg-emerald-50 text-emerald-800 font-semibold"
                    : "border-slate-200 bg-slate-50/50 hover:bg-slate-50 text-slate-600"
                }`}
              >
                <div className={`w-5 h-5 rounded-md border flex items-center justify-center mb-2 ${
                  isChecked ? "border-emerald-600 bg-emerald-600 text-white" : "border-slate-350 bg-white"
                }`}>
                  {isChecked && (
      ...
      {/* Benefits strip */}
      <div className="bg-emerald-50/60 border border-emerald-100 rounded-2xl p-4 flex gap-3 text-xs text-emerald-900">
        <Shield className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
        <div className="space-y-1">
          <strong className="font-bold">Dealer Support & Perks:</strong>
          <p className="leading-relaxed font-medium text-slate-700">
            We provide Tier-1 solar panels, high-efficiency hybrid and grid-tied inverters, smart solar street lights, and durable mounting structures. You get direct access to official brand warranties, wholesale pricing, and promotional materials.
          </p>
        </div>
      </div>
      ...
      <div className="pt-2">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-3.5 bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] text-white font-bold text-sm rounded-xl transition-all shadow-md hover:shadow-emerald-600/15 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isSubmitting ? (
            <>
